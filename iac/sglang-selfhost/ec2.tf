...
locals {
  user_data_script = base64encode(templatefile("${path.module}/user_data.sh", {
    ...
    sglang_context_length      = var.sglang_context_length
    sglang_mem_fraction_static = var.sglang_mem_fraction_static
    sglang_tool_call_parser    = var.sglang_tool_call_parser
    aws_region                 = var.aws_region
    s3_bucket_name             = var.enable_s3_bucket ? aws_s3_bucket.artifacts[0].id : ""
    log_group_name             = var.enable_cloudwatch_monitoring ? aws_cloudwatch_log_group.sglang[0].name : ""
  }))
}
...
resource "aws_cloudwatch_log_group" "sglang" {
  count             = var.enable_cloudwatch_monitoring ? 1 : 0
  name              = "/aws/ec2/${var.project_name}/sglang"
